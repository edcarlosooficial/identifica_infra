...
# A FREE usa apenas conexões TCP simples e explícitas, sem autenticação,
# execução remota, exploração, evasão ou persistência.
...
def parse_targets(value:str)->list[str]:
    value=value.strip()
    if not value:
        raise ValueError("Informe um IP, CIDR ou faixa.")
    if "/" in value:
        net=ipaddress.ip_network(value,strict=False)
        if net.version!=4:
            raise ValueError("Amostra FREE suporta IPv4.")
    ...
    if len(targets)>MAX_HOSTS:
        raise ValueError(f"FREE limitada a {MAX_HOSTS} hosts por execução.")
...
def tcp_probe(ip:str,ports:Iterable[int],timeout:float=.30)->list[int]:
    opened=[]
    for port in ports:
...
def arp_mac(ip:str)->str:
    # Apenas leitura da tabela ARP local já existente.
    try:
        cp=run_cmd(["arp","-a"],3)
...
def os_hint(ttl:Optional[int],ports:list[int])->str:
    ...
    if p.intersection({135,139,445,3389,5985}):
        return "Windows/appliance (heurística)"
    if 22 in p:
        return "Linux/Unix/appliance (heurística)"
    if ttl is not None:
        if ttl<=64:
            return "Linux/Unix/appliance (TTL)"
        if ttl<=128:
            return "Windows/appliance (TTL)"
...
def diagnose(ports:list[int],hostname:str)->list[str]:
    ...
    notes=[]
    if not p:
        notes.append("Host respondeu; nenhuma porta comum da amostra foi detectada.")
    if p.intersection({80,443,8080}):
        notes.append("Serviço web detectado.")
    if 445 in p:
        notes.append("SMB detectado; valide necessidade de exposição no segmento.")
    if 3389 in p:
        notes.append("RDP detectado; mantenha acesso restrito.")
    if 22 in p:
...
def scan(target:str)->list[HostResult]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as ex:
        for r in ex.map(scan_one,targets):
            results.append(r)
...
def export_json(results:list[HostResult],path:str):
    payload={
        ...
        "security_profile":"free-network-mapping-only",
...
def main():
    ...
    ap.add_argument("--authorized",action="store_true",help="Confirma que você está autorizado a mapear o alvo.")
    ap.add_argument("--json",dest="json_path")
    ap.add_argument("--csv",dest="csv_path")
    args=ap.parse_args()

    if not args.authorized:
        ap.error("Use --authorized somente após confirmar que a rede é sua ou que você possui autorização explícita.")

    print("Mareo Tecnologia | Identifica Infra FREE")
    print("Perfil de segurança: somente mapeamento de rede; sem credenciais, execução remota, exploração ou evasão.")
