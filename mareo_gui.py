...
class App(tk.Tk):
    def __init__(self):
        ...
        side=tk.Frame(self,bg="#0B1F33",width=250)
        side.pack(side="left",fill="y")
        side.pack_propagate(False)

        cv=tk.Canvas(side,width=220,height=80,bg="#0B1F33",highlightthickness=0)
        cv.pack(pady=(24,8))
        ...
        tk.Label(
            side,
            text="Mapeamento simples da rede.\nAmostra pública.\n\nLimite: 256 hosts.",
            justify="left",
            bg="#0B1F33",
            fg="#CBD9E2"
        ).pack(anchor="w",padx=24,pady=10)

        main=tk.Frame(self,bg="#F4F7F8")
        ...
        row=tk.Frame(main,bg="#F4F7F8")
        row.pack(fill="x",padx=24,pady=15)
        ...
        self.tree=ttk.Treeview(
            main,
            columns=("ip","status","hostname","mac","os","services"),
            show="headings"
        )
        for col,label,w in [
            ("ip","IP",110),
    ...
    def start(self):
        try:
            count=len(parse_targets(self.target.get()))
        ...
        if not messagebox.askyesno(
            APP_NAME,
            f"Você confirma autorização para analisar {count} host(s)?"
        ):
            return
    ...
    def add_result_row(self, r):
        values=(
            r.ip,
            "ATIVO" if r.online else "-",
            r.hostname,
            r.mac,
            r.os_hint if r.online else "",
            ",".join(r.services),
        )
    ...
    def worker(self):
        try:
            ...
            for r in self.results:
                # Tkinter widgets devem ser atualizados na thread principal.
                # after() aceita somente argumentos posicionais para o callback,
                # por isso encapsulamos a chamada em lambda.
                self.after(0, lambda result=r: self.add_result_row(result))

            self.after(
                0,
                self.status.set,
                f"Concluído: {sum(r.online for r in self.results)} ativo(s) / {len(self.results)}."
            )
        except Exception as e:
    ...
    def export_json(self):
        if not self.results:
            messagebox.showinfo(APP_NAME,"Faça uma análise antes de exportar.")
            return

        path=filedialog.asksaveasfilename(
            defaultextension=".json",
            initialfile="mareo_identifica_infra_free.json",
            filetypes=[("JSON","*.json")]
        )
        if path:
